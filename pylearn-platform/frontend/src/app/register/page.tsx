    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-blue-50 to-white dark:from-dark-bg dark:to-dark-card">
      <div className="w-full max-w-md p-8 bg-white dark:bg-dark-card rounded-xl shadow-lg">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input type="email" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })}
              className="w-full px-4 py-2 border border-gray-300 dark:border-dark-border rounded-lg focus:ring-2 focus:ring-primary-500 dark:bg-dark-card dark:text-white" required />
          </div>
          <div>
        ...
        <p className="mt-6 text-center text-gray-600 dark:text-gray-400">
          Уже есть аккаунт? <Link href="/login" className="text-primary-600 hover:underline">Войти</Link>
        </p>
      </div>
    </div>
  );
